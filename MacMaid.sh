...
if [ "$amiroot" -eq 0 ]
then
    printf "MacMaid Service Require Root Access. Please Enter Your Password.\n"
    sudo -v
    printf "\n"
fi
...
#Install pip-purge
printf "Installing pip-purge.\n"
sudo pip install pip-purge > /dev/null 2>&1

#Purging Python packages
printf "Removing all Python Packages.\n"
sudo pip-purge > /dev/null 2>&1
...
printf "MacMaid Service Took %s this time.\n" "$timed"